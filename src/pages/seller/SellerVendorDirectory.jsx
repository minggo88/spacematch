...
const SellerVendorDirectory = () => {
    ...
    const TYPE_LABELS = {
        popup: t('vendorDirectoryPage.typePopup'),
        gallery: t('vendorDirectoryPage.typeGallery'),
        cafe: t('vendorDirectoryPage.typeCafe'),
        showroom: t('vendorDirectoryPage.typeShowroom'),
        fleamarket: t('vendorDirectoryPage.typeFleamarket'),
    };
    ...
    // Vendor contact access state (for sellers)
    ...
    useEffect(() => {
        fetchVendors();
        fetchPromotions();
        if (user?.role === 'seller') fetchVendorContactAccess();
    }, []);

    const fetchVendorContactAccess = async () => {
        try {
            const res = await fetch(`${API_BASE}/users/vendor_contact_access.php`, { credentials: 'include' });
            ...
            if (data.success) {
                ...
                // Auto-load unlocked vendor contacts
                if (data.viewed_vendor_ids?.length > 0) {
                    for (const vid of data.viewed_vendor_ids) {
                        fetchUnlockedVendorContact(vid);
                    }
                }
            }
        } catch (err) {
            console.error('Vendor contact access fetch failed:', err);
        }
    };

    const fetchUnlockedVendorContact = async (vendorId) => {
        try {
            const res = await fetch(`${API_BASE}/users/unlock_vendor_contact.php?vendor_id=${vendorId}`, { credentials: 'include' });
            const data = await res.json();
            if (data.success && data.already_viewed) {
                setUnlockedVendors(prev => ({ ...prev, [vendorId]: data.contact }));
            }
        } catch (err) { /* ignore */ }
    };

    const handleUnlockVendor = async (vendorId) => {
        setUnlockLoading(true);
        try {
            const res = await fetch(`${API_BASE}/users/unlock_vendor_contact.php`, {
                method: 'POST',
            ...
            if (data.success) {
                ...
                showToast(data.message || t('vendorDirectoryPage.unlockFailed'), 'error');
            }
        } catch (err) {
            showToast(t('vendorDirectoryPage.errorOccurred'), 'error');
        } finally {
    ...
    const allTypes = useMemo(() => {
        const types = new Set();
        vendors.forEach(v => v.types?.forEach(tp => types.add(tp)));
        ...
        if (vendor.has_closed) {
            return (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-gray-100 text-gray-500 border border-gray-200">
                    ...
                    {t('vendorDirectoryPage.recruitDone')}
                </span>
            );
        }
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-8">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                            <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 dark:text-gray-100">{t('vendorDirectoryPage.title')}</h1>
                            <p className="text-sm text-gray-500 dark:text-gray-400">{t('vendorDirectoryPage.subtitle')}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 mb-6">
                    <div className="flex flex-col md:flex-row gap-3">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                placeholder={t('vendorDirectoryPage.searchPlaceholder')}
                    ...
                    {showFilters && (
                        <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-600 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2">{t('vendorDirectoryPage.filterRegion')}</label>
                                <div className="relative">
                                    ...
                                    >
                                        <option value="">{t('vendorDirectoryPage.filterAllRegions')}</option>
                                        {allRegions.map(r => (
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2">{t('vendorDirectoryPage.filterVenueType')}</label>
                                <div className="relative">
                                    <select
                                        value={selectedType}
                                        ...
                                        className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-700 dark:text-gray-200 rounded-xl border border-transparent focus:border-indigo-500 outline-none appearance-none font-medium text-sm"
                                    >
                                        <p className="text-sm text-gray-400 italic mt-2">{t('vendorDirectoryPage.noRegisteredVenues')}</p>
                                        {allTypes.map(tp => (
                            ...
                            {activeFilterCount > 0 && (
                                <div className="md:col-span-2">
                                    ...
                                    >
                                        <X size={14} /> {t('vendorDirectoryPage.filterClear')}</button>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    <div className="mb-8">
                        <div className="bg-gradient-to-r from-amber-50 via-yellow-50 to-amber-50 dark:from-amber-950/30 dark:via-yellow-950/20 dark:to-amber-950/30 rounded-2xl border border-amber-200/60 dark:border-amber-700/40 overflow-hidden shadow-sm">
                            <div className="px-5 py-3 bg-gradient-to-r from-amber-500 to-yellow-500 flex items-center gap-2">
                                ...
                                <span className="text-white font-extrabold text-sm">{t('vendorDirectoryPage.premiumVendors')}</span>
                                <span className="text-white/70 text-xs ml-1">{t('vendorDirectoryPage.premiumTopExposure')}</span>
                            </div>
                            <div className="p-5">
                                            ...
                                            <div className="p-5">
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <h3 className="font-extrabold text-gray-900 dark:text-gray-100 text-lg truncate">{vendor.name}</h3>
                                                        <div className="flex items-center gap-2 mt-1">
                                                            <span className="text-xs text-indigo-600 font-bold">{t('vendorDirectoryPage.venueCount', { count: vendor.venue_count })}</span>
                                                            {vendor.is_verified ? (
                                                                <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-700/50 rounded-full text-[9px] font-extrabold">
                                                                    <BadgeCheck size={9} /> {t('vendorDirectoryPage.verified')}
                                                                </span>
                                                            ) : null}
                                                            {getDdayBadge(vendor)}
                                                        </div>
                ...
                <div className="mb-5">
                    <div className="flex items-center gap-3 mb-3">
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t('vendorDirectoryPage.sponsor')}</span>
                ...
                <div className="flex items-center justify-between mb-4">
                    <p className="text-sm text-gray-500 font-medium">
                        {t('vendorDirectoryPage.totalVendors', { count: filteredVendors.length })}
                    </p>
                </div>
                ...
                {filteredVendors.length === 0 ? (
                    <div className="text-center py-20">
                        ...
                        <h3 className="text-lg font-bold text-gray-400">{t('vendorDirectoryPage.noResults')}</h3>
                        <p className="text-sm text-gray-400 mt-1">{t('vendorDirectoryPage.noResultsHint')}</p>
                    </div>
                ) : (
                                ...
                                >
                                        ...
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-2">
                                                    <h3 className="font-bold text-gray-900 dark:text-gray-100 text-lg truncate">{vendor.name}</h3>
                                                    {vendor.is_featured ? (
                                                ...
                                                <p className="text-xs text-gray-400 truncate">{isAdmin || unlockedVendors[vendor.id] ? vendor.email : '●●●●@●●●●.com'}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-5 space-y-3">
                                        <div className="flex items-center gap-2 text-sm">
                                            ...
                                            <span className="text-gray-600 dark:text-gray-400">{t('vendorDirectoryPage.registeredVenues')} <span className="font-bold text-indigo-600 dark:text-indigo-400">{vendor.venue_count}</span></span>
                                        </div>
                                        {vendor.regions?.length > 0 && (
                                    ...
                                    <div className="px-5 py-3 border-t border-gray-50 dark:border-gray-700 flex items-center justify-between bg-gray-50/50 dark:bg-gray-800/50">
                                        <span className="text-xs text-gray-400">
                                            {t('vendorDirectoryPage.joinDate')} {new Date(vendor.created_at).toLocaleDateString()}
                                        </span>
                                        <div className="flex items-center gap-2">
                                            {getDdayBadge(vendor)}
                                            {(isAdmin || unlockedVendors[vendor.id]) ? (
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); navigate(`/${user.role}/chat?user=${vendor.id}`); }}
                        ...
                        <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden" onClick={e => e.stopPropagation()}>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <span className="inline-block mt-1 px-2.5 py-0.5 rounded-lg text-xs font-bold bg-white/20">
                                                {t('vendorDirectoryPage.vendorRole')}
                                            </span>
                                        </div>
                                    </div>
                            ...
                            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto dark:text-gray-200">
                                ...
                                <div className="p-4 bg-indigo-50 dark:bg-indigo-950/30 rounded-xl border border-indigo-100 dark:border-indigo-800/50">
                                    <p className="text-xs font-bold text-indigo-500 uppercase tracking-wider mb-2">{t('vendorDirectoryPage.vendorIntro')}</p>
                                    {selectedVendor.description ? (
                                        <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-wrap">{selectedVendor.description}</p>
                                    ) : (
                                        <p className="text-sm text-gray-400 italic">{t('vendorDirectoryPage.vendorIntroEmpty')}</p>
                                    )}
                                </div>
                                ...
                                <div className="flex items-center justify-center gap-8 py-3 border-y border-gray-100 dark:border-gray-700">
                                    <div className="text-center">
                                        ...
                                        <p className="text-xs text-gray-400">{t('vendorDirectoryPage.registeredVenues')}</p>
                                    </div>
                                </div>

                                {/* Active Venue List */}
                                {(() => {
                                    const activeVenues = (selectedVendor.venues || []).filter(v => v.is_active !== 0);
                                    const pastVenues = (selectedVendor.venues || []).filter(v => v.is_active === 0);
                                    return (
                                        <>
                                            {activeVenues.length > 0 && (
                                                <div>
                                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                                                        <Building size={12} />
                                                        {t('vendorDirectoryPage.activeVenues')}
                                                    </p>
                                                                    ...
                                                                    <div className="flex items-center justify-between">
                                                                        <div className="flex-1 min-w-0">
                                                                            ...
                                                                            {promo && (
                                                                                ...
                                                                                >
                                                                                    <Flame size={12} />
                                                                                    {t('vendorDirectoryPage.eventOngoing')}
                                                                        ...
                                                                        <div className="flex items-center gap-1 text-indigo-500 group-hover:text-indigo-700 transition-colors flex-shrink-0 ml-2">
                                                                            <span className="text-xs font-medium hidden sm:inline">{t('vendorDirectoryPage.viewDetail')}</span>
                                            ...
                                            {pastVenues.length > 0 && (
                                                <div className="mt-3">
                                                    <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-1">
                                                        <Clock size={12} />
                                                        {t('vendorDirectoryPage.pastVenues')}
                                                    </p>
                                            ...
                                            {activeVenues.length === 0 && pastVenues.length === 0 && (
                                                <p className="text-sm text-gray-400 italic">{t('vendorDirectoryPage.noRegisteredVenues')}</p>
                                            )}
                                        </>
                                    );
                                })()}
                                ...
                                {selectedVendor.regions?.length > 0 && (
                                    <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
                                        <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                                            <MapPin size={12} />
                                            {t('vendorDirectoryPage.activeRegions')}
                                        </p>
                                        <div className="flex flex-wrap gap-1.5">
                                ...
                                {selectedVendor.types?.length > 0 && (
                                    <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
                                        <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                                            <Store size={12} />
                                            {t('vendorDirectoryPage.venueTypeStatus')}
                                        </p>
                                        <div className="flex flex-wrap gap-1.5">
                                            {selectedVendor.types.map((tp, i) => (
                                ...
                                {/* Contact Info */}
                                <div>
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t('vendorDirectoryPage.contactInfo')}</p>
                                    {isAdmin || unlockedVendors[selectedVendor.id] ? (
                                        <>
                                            <div className="grid grid-cols-1 gap-2">
                                                <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                                                    <Mail size={16} className="text-indigo-400" />
                                                    <div>
                                                        <p className="text-xs text-gray-400">{t('vendorDirectoryPage.email')}</p>
                                                        <p className="text-sm font-medium text-gray-800 dark:text-gray-200">{isAdmin ? selectedVendor.email : (unlockedVendors[selectedVendor.id]?.email || selectedVendor.email)}</p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                                                    <Phone size={16} className="text-indigo-400" />
                                                    <div>
                                                        <p className="text-xs text-gray-400">{t('vendorDirectoryPage.phone')}</p>
                                                        <p className={`text-sm font-medium ${(isAdmin ? selectedVendor.phone : unlockedVendors[selectedVendor.id]?.phone) ? 'text-gray-800' : 'text-gray-400 italic'}`}>
                                                            {isAdmin ? (selectedVendor.phone || t('vendorDirectoryPage.notRegistered')) : (unlockedVendors[selectedVendor.id]?.phone || t('vendorDirectoryPage.notRegistered'))}
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                            {/* Chat button for unlocked vendors */}
                                            <button
                                                onClick={() => navigate(`/${user.role}/chat?user=${selectedVendor.id}`)}
                                        ...
                                        <div className="p-5 bg-gradient-to-br from-indigo-900/80 to-violet-900/80 rounded-2xl border border-indigo-500/30 text-center backdrop-blur-sm">
                                            ...
                                            {contactAccess.can_view ? (
                                                <>
                                                    <button
                                                        onClick={() => handleUnlockVendor(selectedVendor.id)}
                                ...
                                {/* Business Info */}
                                <div>
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t('vendorDirectoryPage.businessInfo')}</p>
                                    <div className="grid grid-cols-1 gap-2">
                                        <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                                            <Building size={16} className="text-gray-400" />
                                            <div>
                                                <p className="text-xs text-gray-400">{t('vendorDirectoryPage.businessNo')}</p>
                                                <p className={`text-sm font-medium ${selectedVendor.business_no ? 'text-gray-800' : 'text-gray-400 italic'}`}>
                                                    {selectedVendor.business_no ? (isAdmin ? selectedVendor.business_no : selectedVendor.business_no.slice(0, 3) + '-**-*****') : t('vendorDirectoryPage.notRegistered')}
                                                </p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                                            ...
                                            <div>
                                                <p className="text-xs text-gray-400">{t('vendorDirectoryPage.joinDate')}</p>
                                                <p className="text-sm font-medium text-gray-800 dark:text-gray-200">{new Date(selectedVendor.created_at).toLocaleDateString()}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden" onClick={e => e.stopPropagation()}>
                                ...
                                {(() => {
                                    ...
                                    return imgs.length > 0 ? (
                                        ...
                                        <div className="w-full h-full flex flex-col items-center justify-center text-gray-400">
                                            ...
                                            <span className="mt-2 text-sm">{t('vendorDirectoryPage.noImages')}</span>
                                        </div>
                                    );
                                })()}
                            ...
                            <div className="p-6 space-y-4 max-h-[50vh] overflow-y-auto dark:text-gray-200">
                                ...
                                {selectedVenue.price !== undefined && selectedVenue.price !== null && (
                                    <div className="p-4 bg-indigo-50 dark:bg-indigo-950/30 rounded-xl border border-indigo-100 dark:border-indigo-800/50">
                                        <p className="text-xs font-bold text-indigo-500 uppercase tracking-wider mb-1">{t('vendorDirectoryPage.priceLabel')}</p>
                                        <div className="flex items-end gap-1">
                                            {Number(selectedVenue.price) === 0 ? (
                                                <span className="text-2xl font-bold text-emerald-600">{t('vendorDirectoryPage.priceFree')}</span>
                                            ) : (
                                                <>
                                                    ...
                                                    <span className="text-sm text-gray-400 mb-0.5">
                                                        {selectedVenue.pricing_unit === 'daily' ? t('vendorDirectoryPage.priceDaily') : selectedVenue.pricing_unit === 'weekly' ? t('vendorDirectoryPage.priceWeekly') : selectedVenue.pricing_unit === 'monthly' ? t('vendorDirectoryPage.priceMonthly') : ''}
                                                    </span>
                                                </>
                                            )}
                                        </div>
                                        {parseFloat(selectedVenue.commission_rate) > 0 && (
                                            <p className="text-xs text-orange-600 mt-2 font-medium">{t('vendorDirectoryPage.commissionRate', { rate: selectedVenue.commission_rate })}</p>
                                        )}
                                    </div>
                                )}
                                ...
                                {/* Description */}
                                <div>
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t('vendorDirectoryPage.venueIntro')}</p>
                                    <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-wrap">
                                        {selectedVenue.description || t('vendorDirectoryPage.venueIntroEmpty')}
                                    </p>
                                </div>
                                ...
                                >
                                    <ArrowLeft size={16} />
                                    {t('vendorDirectoryPage.backToVendor')}</button>
                            </div>
                        </div>
                    </div>
                )}
