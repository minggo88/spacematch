...
try {
    ...
    // Get vendor info (use session-cached column availability)
    $cols = "id, name, email, phone";
    if (empty($_SESSION['_vendor_extra_cols'])) {
        $_SESSION['_vendor_extra_cols'] = [];
        foreach (['instagram', 'business_no'] as $oc) {
            try {
                $chk = $conn->query("SHOW COLUMNS FROM users LIKE '{$oc}'");
                if ($chk && $chk->fetch())
                    $_SESSION['_vendor_extra_cols'][] = $oc;
            } catch (PDOException $e) {
            }
        }
    }
    foreach ($_SESSION['_vendor_extra_cols'] as $oc) {
        $cols .= ", {$oc}";
    }

    $vendorStmt = $conn->prepare("SELECT {$cols} FROM users WHERE id = ? AND role = 'host'");
    $vendorStmt->execute([$host_id]);
    ...
    if ($access_start && $access_end) {
        $alreadyStmt = $conn->prepare("SELECT id FROM vendor_contact_views WHERE seller_id = ? AND host_id = ? AND viewed_at >= ? AND viewed_at <= ?");
        $alreadyStmt->execute([$seller_id, $host_id, $access_start . ' 00:00:00', $access_end . ' 23:59:59']);
        $already_viewed = $alreadyStmt->fetch();
        $usedStmt = $conn->prepare("SELECT COUNT(DISTINCT host_id) as used FROM vendor_contact_views WHERE seller_id = ? AND viewed_at >= ? AND viewed_at <= ?");
        $usedStmt->execute([$seller_id, $access_start . ' 00:00:00', $access_end . ' 23:59:59']);
    } else {
        $month_start = date('Y-m-01 00:00:00');
        $alreadyStmt = $conn->prepare("SELECT id FROM vendor_contact_views WHERE seller_id = ? AND host_id = ? AND viewed_at >= ?");
        $alreadyStmt->execute([$seller_id, $host_id, $month_start]);
        $already_viewed = $alreadyStmt->fetch();
        $usedStmt = $conn->prepare("SELECT COUNT(DISTINCT host_id) as used FROM vendor_contact_views WHERE seller_id = ? AND viewed_at >= ?");
    ...
    // Record the view
    $insertStmt = $conn->prepare("INSERT INTO vendor_contact_views (seller_id, host_id) VALUES (?, ?)");
    $insertStmt->execute([$seller_id, $host_id]);
    ...
    error_log('[unlock_host_contact] ' . $e->getMessage());
    echo json_encode(["success" => false, "message" => "연락처 열람 중 오류가 발생했습니다."]);
}
?>