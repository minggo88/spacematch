 ...
 * Mirror of seller_contact_access.php but for sellers viewing hosts
 * 
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        ...
        if ($access_start && $access_end) {
            $viewStmt = $conn->prepare("SELECT COUNT(DISTINCT host_id) as used FROM vendor_contact_views WHERE seller_id = ? AND viewed_at >= ? AND viewed_at <= ?");
            $viewStmt->execute([$seller_id, $access_start . ' 00:00:00', $access_end . ' 23:59:59']);
        } else {
            $month_start = date('Y-m-01 00:00:00');
            $viewStmt = $conn->prepare("SELECT COUNT(DISTINCT host_id) as used FROM vendor_contact_views WHERE seller_id = ? AND viewed_at >= ?");
        ...
        // Viewed vendor IDs
        if ($access_start && $access_end) {
            $viewedStmt = $conn->prepare("SELECT DISTINCT host_id FROM vendor_contact_views WHERE seller_id = ? AND viewed_at >= ? AND viewed_at <= ?");
            $viewedStmt->execute([$seller_id, $access_start . ' 00:00:00', $access_end . ' 23:59:59']);
        } else {
            $month_start = date('Y-m-01 00:00:00');
            $viewedStmt = $conn->prepare("SELECT DISTINCT host_id FROM vendor_contact_views WHERE seller_id = ? AND viewed_at >= ?");
            $viewedStmt->execute([$seller_id, $month_start]);
        }
        $viewed_hosts = $viewedStmt->fetchAll(PDO::FETCH_COLUMN);
