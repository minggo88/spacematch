<?php
/**
 * Vendor Tax Management API
 * 
 * Provides tax calculations and summaries for vendor users.
 * Nearly identical to seller_tax.php but with:
 *   - role check: 'host' instead of 'seller'
 *   - table: vendor_tax_settings (instead of seller_tax_settings)
 *   - revenue data from vendor_stats table
 * 
 * Actions:
...
// ── Tax rules by country (identical to seller_tax.php) ──
$TAX_RULES = [
    'KR' => [
        'country' => 'South Korea',
        'country_local' => '한국',
...
// ── Auto-migrate: create vendor_tax_settings table ──
try {
    $conn->query("SELECT 1 FROM vendor_tax_settings LIMIT 1");
} catch (PDOException $ex) {
    $conn->exec("CREATE TABLE vendor_tax_settings (
...
try {
    switch ($action) {
        ...
        case 'calculate':
            ...
            $stmt = $conn->prepare("SELECT * FROM vendor_tax_settings WHERE user_id = ? AND country_code = ?");
        ...
        case 'save_settings':
            ...
            $stmt = $conn->prepare("SELECT id FROM vendor_tax_settings WHERE user_id = ? AND country_code = ?");
            $stmt->execute([$userId, $country]);
            $existing = $stmt->fetch(PDO::FETCH_ASSOC);

            if ($existing) {
                $stmt = $conn->prepare("UPDATE vendor_tax_settings SET 
                ...
                $stmt = $conn->prepare("INSERT INTO vendor_tax_settings 
        ...
        case 'reset_settings':
            $country = strtoupper(trim($input['country'] ?? 'KR'));
            $stmt = $conn->prepare("DELETE FROM vendor_tax_settings WHERE user_id = ? AND country_code = ?");
            $stmt->execute([$userId, $country]);
        ...
        case 'summary':
            ...
            $country = strtoupper(trim($input['country_code'] ?? $_GET['country_code'] ?? ''));

            $where = "WHERE user_id = ? AND record_date LIKE ?";
            $params = [$userId, $year . '%'];
            ...
            $stmt = $conn->prepare("
                ...
                FROM vendor_stats 
                $where
                GROUP BY SUBSTRING(record_date, 1, 7), currency, country_code
                ORDER BY month
            ");
            $stmt->execute($params);
